...
PM2Listener.prototype._addCompatiblePm2 = function(pm2Interface, version, pm2Details) {
	...
	const getSystemData = () => {
		pm2Interface.rpc.getMonitorData({}, (error, data) => {
            if(error) {
                this._logger.warn(["PM2Listener", "Error retrieving monitor data", error.message]);
            } else {
                // only expose fields we are interested in
                this._mapMonitorData(pm2Interface, data, pm2Details);
            }
		});
		pm2Interface.rpc.getSystemData({}, (error, data) => {
			if(error) {
				this._logger.warn(["PM2Listener", "Error retrieving system data", error.message]);
			} else {
				// only expose fields we are interested in
				let systemData = this._mapSystemData(pm2Interface, data, pm2Details);
...
PM2Listener.prototype._mapMonitorData = function(pm2Interface, data, pm2Details) {
    this._processes = [];
    let reloading = new Set();
    let now = Date.now();
    data.forEach((process) => {
        if((typeof process.pm_id == "string" || process.pm_id instanceof String) && process.pm_id.substring(0, 8) == "todelete") {
            ...
            reloading.add(parseInt(process.pm_id.substring(8), 10));
        ...
        this._processes.push({
            ...
            uptime: (now - process.pm2_env.pm_uptime) / 1000,
...
PM2Listener.prototype._mapSystemData = function(pm2Interface, data, pm2Details) {
	...
	let systemData = {
		name: pm2Interface.bind_host,
		inspector: pm2Details.inspector||'unknown',
		system: {
